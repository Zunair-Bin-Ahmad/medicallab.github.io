...
if (isset($_POST['submit'])) {
  ...
  if ($imagepath=='') {
     $query="UPDATE `add_report` SET `Date`='$date',`Time`='$time',`Ptname`='$ptname',`Image`='$image',`Labno`='$labno' WHERE id='$id'";
  ...
  $imagepath="images/".$_FILES['image']['name'];
    ...
    $query="UPDATE `add_report` SET `Date`='$date',`Time`='$time',`Ptname`='$ptname',`Image`='$imagepath',`Labno`='$labno' WHERE id='$id'";
    $run=mysqli_query($connect,$query);
    if (!$run) {
      echo "<script>alert('Data Not Inserted')</script>";
    }
        ...
        <div class="page-wrapper"> 
            <div class="content">
                ...
                <div class="row">
                    <div class="col-lg-8 offset-lg-2">
                       <?php 
                      $query="SELECT *from `add_report` WHERE id=$id";
                      $run=mysqli_query($connect,$query);
                  ...
                  <input type="Time" class="form-control" name="time" value="<?php echo $row['Time'] ?>">
                   ...
                   <div class="form-group">
                    <label for="password-1">Pt Name</label>
                  <input type="text" class="form-control" name="ptname" value="<?php echo $row['Ptname'] ?>">
                  </div>
                  ...
                  <input type="text" class="form-control" name="labno" value="<?php echo $row['Labno'] ?>">
                  </div>

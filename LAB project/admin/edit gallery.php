...
if (isset($_POST['submit'])) {
  ...
  if ($imagepath=='') {
     $query="UPDATE `add_gallery` SET `Date`='$date',`Time`='$time',`Title`='$title',`Image`='$image' WHERE id='$id'";
  ...
  $imagepath="images/".$_FILES['image']['name'];
    ...
    $query="UPDATE `add_gallery` SET `Date`='$date',`Time`='$time',`Title`='$title',`Image`='$imagepath' WHERE id='$id'";
    $run=mysqli_query($connect,$query);
    if (!$run) {
      echo "<script>alert('Data Not Inserted')</script>";
    }
        ...
        <div class="page-wrapper"> 
            <div class="content">
                ...
                <div class="row">
                    <div class="col-lg-8 offset-lg-2">
                      <?php 
                      $query="SELECT *from `add_gallery` WHERE id=$id";
                      $run=mysqli_query($connect,$query);
